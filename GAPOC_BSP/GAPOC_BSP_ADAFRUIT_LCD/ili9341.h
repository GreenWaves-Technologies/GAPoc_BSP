...
void drawChar(spi_t *spim,int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg, uint8_t size);
...
void writePixels(spi_t *spim,uint16_t * colors, uint32_t len);
void drawRGBBitmap(spi_t *spim,signed short x, signed short y,  unsigned short *pcolors, signed short w, signed short h) ;
