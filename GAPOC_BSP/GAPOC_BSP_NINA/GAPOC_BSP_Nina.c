...
void GAPOC_NINA_Uart_Set_Baud(uint32_t baudrate)
{
    serial_baud(&hUart, baudrate);
}
// -----------------------------------------------------
...
void GAPOC_NINA_Get_ByteArray_Blocking(const uint8_t* pArray, uint32_t Num_Items)
{
    UART_TransferReceiveBlocking(uart_addrs[0], (unsigned char*)pArray, Num_Items);   // receive first char
    printf("Getting to BLE : \n");
    for (uint32_t i=0; i<Num_Items; i++)
    {
        printf("%2x ", pArray[i]);
    }
 ...
 */
void GAPOC_NINA_Exit_Data_Mode()
{
    UART_WriteByte(uart_addrs[0], '+');
    UART_WriteByte(uart_addrs[0], '+');
    UART_WriteByte(uart_addrs[0], '+');
}
